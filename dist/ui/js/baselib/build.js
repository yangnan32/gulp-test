...
zWidgets.initToolPage = function(data) {
	var toolPage = {
		pagerid 			: data.pagerid, //divID
		...
		hrefLatter			: '', //链接尾部
		skin				:"",
		gopageWrapId		: 'page_gopage_wrap',
		gopageButtonId		: 'page_btn_go',
		gopageTextboxId		: 'page_btn_go_input',
		lang				: {
			firstPageText			: '首页',
			firstPageTipText		: '首页',
			lastPageText			: '尾页',
		...
		click	: function(n){
			//这里自己实现
			//这里可以用this或者toolPage访问toolPage对象
			return false;
		},
		...
		init : function(config,enforceInit){
			...
			var gopage_info = '';
			if(this.isGoPage && this.total > 1){
				gopage_info = ' , <span class="goPageBox">'+this.lang.gopageBeforeText+'<span class="widget-toolPage_gopage_wrap" id="'+this.gopageWrapId+'">'+
					'<input type="button" id="'+this.gopageButtonId+'" class="nowPageBtnClick widget-toolPage_btn_go" value="'
						+this.lang.gopageButtonOkText+'" />'+
					'<input type="text" id="'+this.gopageTextboxId+'" class="nowPageTextClick widget-toolPage_btn_go_input" value="'+this.pno+'" /></span>'+this.lang.gopageAfterText+'</span>';
			}
			//分页处理
			if(this.total <= 8){
			...
			var pagerHtml = '<div id="'+this.pagerid+'Page" class="widget-toolPage toolPage-'+this.skin+'">';
			...
			$("#"+ this.pagerid + "Page").width($("#"+this.pagerid+" .pageBtnWrap").width()+$("#"+ this.pagerid + " .infoTextAndGoPageBtnWrap").width() + 10);
			$(document).off("."+this.pagerid);
			$(document).on("click."+this.pagerid, "#"+this.pagerid+" .pageToolClick", function(){
				that._clickHandler($(this).attr("myPage"));
			}).on("click."+this.pagerid, "#"+this.pagerid+" .nowPageBtnClick", function(){
				var n = parseInt($(this).next("input").val());
				if(n < 1) n = 1;
				if(n > that.total) n = that.total;
				that._clickHandler(n);
			}).on("keyup."+this.pagerid, "#"+this.pagerid+" .nowPageTextClick", function(e){
				that.keypress_gopage(e);
			}).on("focus."+this.pagerid, "#"+this.pagerid+" .nowPageTextClick", function(e){
				that.focus_gopage(e);
			}).on("blur."+this.pagerid, "#"+this.pagerid+" .nowPageTextClick", function(e){
				that.blur_gopage(e);
			}).on("selectPage."+this.pagerid, "#"+this.pagerid, function(e, page){
				that.selectPage(e);
			});
		...
		_initConfig : function(config){
			...
			this.limit = isNaN(config.limit) ? 20 : parseInt(config.limit);
			this.totalRecords = isNaN(config.totalRecords) ? 0 : parseInt(config.totalRecords);
			this.total = Math.ceil(parseInt(this.totalRecords)/this.limit);
			if(config.pagerid){this.pagerid = config.pagerid;}
			if(config.mode){this.mode = config.mode;}
			if(config.skin){this.skin = config.skin;}
			if(config.gopageWrapId){this.gopageWrapId = config.gopageWrapId;}else{this.gopageWrapId = config.pagerid+"GopageWrap"}
			if(config.gopageButtonId){this.gopageButtonId = config.gopageButtonId;}else{this.gopageButtonId = config.pagerid+"BtnGo"}
			if(config.gopageTextboxId){this.gopageTextboxId = config.gopageTextboxId;}else{this.gopageTextboxId = config.pagerid+"BtnGoInput"}
			...
			this.inited = true;
			$("#"+config.id).append($("#"+this.pagerid));
		},
		_getHandlerStr : function(n){
			if(this.mode == 'click'){
				// return 'href="'+this.getHref(n)+'" onclick="return zWidgets.toolPage._clickHandler('+n+')"';
				return 'class="pageToolClick" myPage="'+n+'"' 
			}
			//link模式，也是默认的
